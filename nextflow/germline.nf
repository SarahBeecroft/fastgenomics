...
process AlignAndSort {
    publishDir "${params.outputDir}/QCMetrics/${sampleID}/Markduplicates", mode: "${saveMode}", pattern: "*.duplicate_metrics"
    publishDir "${params.outputDir}/${sampleID}/", mode: "${saveMode}", pattern: "large_files/*.bam*"

    input:
    each line from lines

    output:
    set val(sampleID),file("large_files/${output_bam_basename}.bam"),file("large_files/${output_bam_basename}.bam.bai") into HaplotypeCallerInChannel
    ...
    fastq1 = list[1]
    fastq2 = list[2]
    output_bam_basename = "${sampleID}.dedup.merged.sorted.hs38"
